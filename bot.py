...
# In-memory tracking cache for link streaks
LINK_SPAM_TRACKER = {}
...
async def group_link_guardian_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_link:
        if await check_admin_privileges(update, context):
            return  
        ...
        if current_strikes > 5:
            ...
            try:
                ...
                logger.error(f"Mute failure: {mute_err}")
                muted_status = "❌ <b>FAILED TO MUTE (Check Perms)</b>"

            report_log = (
                f"🚨 <b><u>LINK SPAM PUNISHMENT TRIGGERED</u></b>\n\n"
                ...
                f"💬 <i>User restricted after posting more than 5 sequential links.</i>"
            )
            try:
